...
import { useRoleBasedAccess, PermissionType } from '../contexts/rbac'
...
export default function SidebarWithContentSeparator() {
  ...
  // Define which menu items should be shown based on permissions
  const showDashboard = hasPermission('view_dashboard')
  const showEfetivo = hasPermission('view_users')
  const showProcessos = hasPermission('view_vacations')
  const showAtestados = hasPermission('view_sick_notes')
  const showFerias = hasPermission('view_vacations')
  const showCursos = hasPermission('view_courses')
  const showWeapons = hasPermission('view_weapons')
  const showCargaFixa = hasPermission('manage_weapons')
  const showDashboardWeapons = hasPermission('view_weapons')
  const showControleArmas = hasPermission('manage_weapons')
  const showCadastrarArmas = hasPermission('manage_weapons')
  const showCuidandoDoCuidador = true // This section is accessible to all roles
    ...
    <>
      <div className="hidden sm:block md:block xl:block  h-[calc(100vh)] dark:bg-slate-800 bg-caqui-200 sticky top-0 w-full max-w-[17rem]  p-4  ">
        ...
        <List>
          ...
          {/* Gestão de Material Bélico Section */}
          <Accordion
            open={open === 4}
            icon={
              <ChevronDownIcon
